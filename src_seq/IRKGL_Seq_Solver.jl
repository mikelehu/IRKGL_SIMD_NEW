...
struct IRKGL_Seq{s,  initial_interp, m,myoutputs,nrmbits,backward} <: IRKAlgorithm{s, initial_interp, m,myoutputs,nrmbits,backward} end
IRKGL_Seq(;s=8, initial_interp=1, m=1,myoutputs=false,nrmbits=0,backward=false)=IRKGL_Seq{s, initial_interp, m,myoutputs,nrmbits,backward}()

function DiffEqBase.__solve(prob::DiffEqBase.AbstractODEProblem{uType,tType,isinplace},
    alg::IRKGL_Seq{s,initial_interp, m,myoutputs,nrmbits,backward}, args...;
    dt,
    saveat=[],
    save_everystep=true,
    adaptive=false,
 ...
 #   println("Integrazio berria dt=",dt)
    ...
    indices=1:length_u
     ...
     irkgl_cache = IRKGL_Cache(f,p,b,c,mu,nu,nu1,nu2,theta,omega,alpha,g,d,
                               U, U_, L, dU, 
    ...
    for tj1 in tstops
        ...
        while cont
             ...
             for i in 1:m
                ...
                irkgl_cache.step_number[] += 1

                if (irkgl_cache.step_number[] ==1)
                
                    (status,j_iter) = IRKGLstep_fixed!(tj,tj1,uj,ej,prob,dts,irkgl_cache)
                    ...
                    irkgl_cache.tau[]=(dt*sqrt(E2))^(1/(s-1))
 
                    trace ? println("tau=", tau[]) : nothing

                else
                    (status,j_iter) = IRKGLstep_adap!(tj,tj1,uj,ej,prob,dts,irkgl_cache)
                end
